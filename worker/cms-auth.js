/**
 * GitHub OAuth handler for Sveltia CMS.
 *
 * Vendored from sveltia/sveltia-cms-auth (MIT), v0.1.0, with one change: the
 * default export below is consumed by worker/index.js rather than being the
 * Worker entry point itself, so this runs on our own domain instead of a
 * second Worker. Keeping it byte-similar to upstream means security fixes can
 * be pulled in by diffing against the source repository.
 *
